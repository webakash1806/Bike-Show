...
import { useUpdateHelmetMutation } from '../../../Redux/API/HelmetApi'
...
import { Helmet, ResponseData } from '../../../interfaces/interface'



const EditHelmet = ({ data, onClose }: { data: Helmet, onClose: (boolean: boolean) => void }) => {

    const [updateHelmet, { isLoading }] = useUpdateHelmetMutation()

    const [helmetDataCount, setHelmetDataCount] = useState<number>(data.helmet.length)
    console.log(data)
    const [variantCount, setVariantCount] = useState<number[]>(
        data.helmet.map(h => h.variant.length)
    )
            ...
            if (helmet.helmet[index]?.uniqueId === "") {
                ...
                setHelmet(prev => ({
                    ...prev, helmet: [
                        ...
                        { ...prev.helmet[index], uniqueId: uniqueId, helmetImg: { url: URL.createObjectURL(selectedFile), publicId: fileName } },
                ...
                setHelmet(prev => ({
                    ...prev, helmet: [
                        ...prev.helmet.slice(0, index),
                        { ...prev.helmet[index], uniqueId: unique, helmetImg: { url: URL.createObjectURL(selectedFile), publicId: fileName } },
                        ...prev.helmet.slice(index + 1)
                    ]
                }));
                setFile((prevFiles: File[] | null) => {
    ...
    const handleRemoveImage = (index: number) => {
        setHelmet(prev => ({
            ...prev, helmet: [
                ...prev.helmet.slice(0, index),
                { ...prev.helmet[index], uniqueId: "", helmetImg: { url: "", publicId: "" } },
                ...prev.helmet.slice(index + 1)
            ]
        }));
        setFile((prevFiles: File[] | null) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        formData.append("helmet", JSON.stringify(helmet.helmet.map(({ helmetImg, ...rest }) => rest)))

        file?.forEach((file) => {
            formData.append("helmetImg", file as File)
        })

        const response = await updateHelmet({ formData, id: data?._id }) as { data: ResponseData }
    ...
    return (
        <form onSubmit={handleSubmit} noValidate className="relative w-[94vw] sm:w-[96vw]  p-4 sm:p-6  pt-2 sm:pt-2 sm:max-w-[50rem] max-w-[28rem] mx-auto  space-y-4 overflow-hidden rounded-t shadow-lg bg-[#fffefe] rounded-md">
            ...
            <h1 className="mx-auto mb-6 text-xl font-semibold w-fit">Edit helmet</h1>
            ...
            <div className="grid grid-cols-1 gap-2 sm:gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                    ...
                    <SelectNative onChange={handleChange} name="helmetType" id={"helmetType"} defaultValue={helmet?.helmetType}>
                    ...
                    <div key={index} className="relative p-2 space-y-3 bg-gray-100 border border-gray-100 rounded shadow-md">
                            ...
                            return <div key={i} className="flex items-end gap-1 ">
                                <div className="space-y-2 w-[7rem] mr-2">
                                    ...
                                    <SelectNative onChange={(e) => { handleVariantChange(e, index, i) }}
                                        name="size" id={"size"} defaultValue={helmet.helmet[index]?.variant[i]?.size || ""}>
                                        <option value="" disabled>
                                            Select size
                                        </option>
                                ...
                                <div className="w-full space-y-1">
                                    ...
                                    <div className="relative flex border rounded-md border-neutral-200">
                                        ...
                                        <Input
                                            ...
                                            value={helmet.helmet[index]?.variant[i]?.price || 0}
                                            name="price"
                                        />
                                    </div>
                                </div>
                        ...
                        <div >
                            ...
                            <div className="relative flex items-center mt-1 size-20 group">
                                {helmet.helmet[index]?.helmetImg?.url ? (
                                    <img
                                        src={helmet.helmet[index]?.helmetImg?.url}
                                        alt={``}
