...
import { useAddScootyMutation } from '../../../Redux/API/ScootyApi';
...
const AddScootyForm = ({ onScootyIdChange, onTabChange }: { onScootyIdChange: (id: string | null) => void, onTabChange: (tab: number) => void }) => {
    const [addScooty, { isLoading }] = useAddScootyMutation()

    const [scooty, setScooty] = useState<AddScootyProps>({
        name: '',
        price: 0,
        description: '',
        previewImage: null
    ...
    const handleScootyForm = async (e: React.FormEvent) => {
        ...
        const response = await addScooty({ formData, id: "" }) as { data: BikeResponse }


        onScootyIdChange(response?.data?.scooty?._id)
    ...
    return (
        <form onSubmit={handleScootyForm} noValidate className="relative w-[94vw] sm:w-[96vw]  p-4 sm:p-6  pt-2 sm:pt-2 sm:max-w-[50rem] max-w-[28rem] mx-auto  space-y-4 overflow-hidden rounded-t shadow-lg bg-[#fffefe] rounded-md">
            {isLoading && <Loaders isLoading={isLoading} />}
            <h1 className="mx-auto mb-6 text-xl font-semibold w-fit">Add new scooty</h1>
