...
import { FaCamera, FaPlusSquare, FaTrashAlt } from "react-icons/fa";
import { HiXMark } from "react-icons/hi2";
import { v4 as uuidv4 } from 'uuid';
import Loaders from "../../Loaders";
import { useAddScootyOthersMutation } from "../../../Redux/API/ScootyApi";
...
export default function ColorsFeaturesForm({ scootyId }: { scootyId: string | Blob | null, onTabChange: (tab: number) => void }) {
    const [addScootyOthers, { isLoading }] = useAddScootyOthersMutation()
    const [colorDataCount, setColorDataCount] = useState(1)
    ...
    const [featuresDataCount, setFeaturesDataCount] = useState(1)
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        formData.append("scootyId", scootyId as string)
        const res = await addScootyOthers({ formData })
        ...
        <div className="relative w-[94vw] sm:w-[96vw]  p-4 sm:p-6  pt-2 sm:pt-2 sm:max-w-[50rem] max-w-[28rem] mx-auto mb-10 space-y-4 overflow-hidden rounded-t shadow-lg bg-[#fffefe] rounded-md">
            ...
            <Accordion type="single" collapsible className="w-full" defaultValue="1">
                ...
                <button onClick={handleSubmit} className="relative mb-4 w-full font-semibold flex items-center justify-center px-4 gap-3 py-2.5 text-sm text-white transition duration-200 bg-red-600 rounded-md top-2">
                    Add Color & Features <FaPlusSquare className="text-[1.1rem]" />
                </button>
            </Accordion>
        </div>
    );
