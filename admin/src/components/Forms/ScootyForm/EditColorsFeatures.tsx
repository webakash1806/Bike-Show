...
import { Image, ResponseData } from "../../../interfaces/interface";
...
export default function EditColorsFeaturesForm({ scootyId, editColor, editFeature, onTabChange, onClose }: { scootyId: string, editColor: Color[], editFeature: features[], onTabChange: (tab: number) => void, onClose: (boolean: boolean) => void }) {
    const [addScootyOthers, { isLoading }] = useAddScootyOthersMutation()
    // const [addBikeFeatures] = useAddBikeFeaturesMutation()
    ...
    const [colorData, setColorData] = useState<Color[]>(editColor.length > 0 ? editColor?.map(color => ({
        colorName: color.colorName,
        colorCode: color.colorCode,
        uniqueId: color.uniqueId,
        img: color?.colorScootyImg?.url || ""
    })) : [{
        colorName: "",
        colorCode: "",
        uniqueId: "",
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        formData.append("scootyId", scootyId)
        const res = await addScootyOthers({ formData }) as { data: ResponseData }
