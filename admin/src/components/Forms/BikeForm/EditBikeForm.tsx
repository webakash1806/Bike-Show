...
import { useUpdateBikeMutation } from '../../../Redux/API/BikeAPI';
...
const EditBikeForm = ({ bikeData, onTabChange }: { bikeData: any, onTabChange: (tab: number) => void }) => {
    ...
    const [bike, setBike] = useState<AddBikeProps>({
        ...
        previewImage: bikeData?.image?.url || null
    });
    ...
    const handleBikeForm = async (e: React.FormEvent) => {
        ...
        const response = await updateBike({ formData, id: bikeData?._id }) as { data: ResponseData }
    ...
    return (
        <form onSubmit={handleBikeForm} noValidate className="relative w-full  p-4 sm:p-6  pt-2 sm:pt-2  mx-auto mb-10 space-y-4 overflow-hidden rounded-t shadow-lg bg-[#fffefe] rounded-md">
            {isLoading && <Loaders isLoading={isLoading} />}

            <h1 className="mx-auto mb-6 text-xl font-semibold w-fit">Edit bike</h1>
